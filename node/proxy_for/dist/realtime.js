...
app.get("/gps", cache("5 seconds"), async (req, res) => {
    try {
        const time = Date.now();
            ...
            try {
                ...
                if (elronResponse.ok) {
                    ...
                    if (elronText && elronText.length > 0) {
                        const elronJson = JSON.parse(elronText);
...
app.get("/stops/:stopId/departures", async (req, res) => {
    try {
        ...
        const url = "https://transport.tallinn.ee/siri-stop-departures.php?stopid=" + stopId;
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 2000);
        // After given time abort connection.
        let fetchPromise = fetch(url, { signal: controller.signal }).finally(() => clearTimeout(timeout));
        ...
        if (response.ok) {
            ...
            result = departures
                .map((departure) => {
                const rt = realtimeDepartures.find((rtDep) => rtDep.scheduleTime === departure.departure_time &&
