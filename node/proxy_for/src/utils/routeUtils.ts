...
async function getRouteCoordinatesFromDB({ line, destination, type }: { line: string; destination: string; type: string }) : Promise<Array<[number, number]>> {
  // Map type if needed
  ...
  if (shape.shape_geom) {
    ...
    if (/^[0-9A-Fa-f]+$/.test(wkt.trim())) {
      ...
      const geom = wkx.Geometry.parse(Buffer.from(wkt, "hex")) as any;
      if (geom && geom.points) {
          coordinates = geom.points.map((pt: any) => [pt.y, pt.x]);
      }
    } else {
...
export async function getRouteStops(line: string, destination: string, type: string) {
  type = typeMapper[type] || type;
