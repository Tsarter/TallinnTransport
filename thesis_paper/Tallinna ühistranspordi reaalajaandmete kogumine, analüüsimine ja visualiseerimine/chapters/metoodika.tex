...
\begin{figure}[H] % h-here, if possible; H-here, definitely; t-top of the page; b-bottom of the page; p-page of floats
    \centering
    \includegraphics[width=.8\textwidth]{figures/timescaleDBSQL.png} % Includes an image from the specified path, setting the width to 50% of the text width
    \caption{TimescaleDB optimeerimise SQL käsud. Sarnased andmed viiakse üksteisele andmebaasis lähemale ehk konkreetse sõiduki asukohad on järjest ning reapõhised andmed muudetakse 7 päeva möödudes tulbapõhisteks. } % Provides a caption for the figure, italicizing the text
    \label{fig:TimescaleDB} % Sets a label for the figure to be referenced later
\end{figure}
...
\begin{figure}[H] % h-here, if possible; H-here, definitely; t-top of the page; b-bottom of the page; p-page of floats
    ...
    \caption{Bussiliini 1 asukoha andmed 16. märts 2025. Viru Keskuse maaaluse peatuse tõttu on sealsed asukohad ebatäpsed. Samuti on osad punktid Narva maantee ääres olevate majade peal. Ilmselt signaali seintelt peegeldumise tõttu.} % Provides a caption for the figure, italicizing the text
...
\begin{figure}[H] % h-here, if possible; H-here, definitely; t-top of the page; b-bottom of the page; p-page of floats
    ...
    \caption{Trammiliini 4 marsruudid hobujaama juures. Kurvi pikkus on 170 m, kuid otse minnes 120 m. Trajektoorid ei jälgi täpselt kurvi, vaid lõikavad seda sirgemaks. Keskmiste kiiruste arvutamisel tuleb siit viga.} % Provides a caption for the figure, italicizing the text
